...
LOAD_TEXTUREBMP_RESULT loadOpenGL2DTextureBMP(const char* filename, 
                                              ...
                                              bool showErrors)
{
  // The error checking below assumes that there are no uncaught OpenGL errors
  // at this point. The caller should verify this, but for now, we simply
  // flush the error buffer.
  GLenum e = glGetError();
  while (e != GL_NO_ERROR)
  {
    if (showErrors)
		cerr << "Error before loadOpenGL2DTextureBMP starts:\n\t"
             << (char*)gluErrorString(e) << endl;
    e = glGetError();
   }
  ...
  if (e!=GL_NO_ERROR)
  {
    ...
    {
      if (showErrors)
         cerr << "Error during loadOpenGL2DTextureBMP:\n\t"
              << (char*)gluErrorString(e) << endl;
      e = glGetError();
    }
